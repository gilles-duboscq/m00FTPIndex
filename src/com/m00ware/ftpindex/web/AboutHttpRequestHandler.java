...
public class AboutHttpRequestHandler extends BaseHttpRequestHandler {
    ...
    public void doHandle(HttpRequest request, HttpResponse response, HttpContext context) throws HttpException, IOException {
        ...
        lines.add(new WebText(""));
        lines.add(new WebText(
                              "    This m00FTPIndex server searches for FTP servers with anonymous access within the " + this.getBackEnd().getScanner()
                                                                                                                             .getRange() + " IP range and indexes their content. Content may have changed since the FTPs were last indexed."));
        lines.add(new WebText(""));
        lines.add(new WebText(
