...
import com.m00ware.ftpindex.search.BasicSearchIndex;
import com.m00ware.ftpindex.search.SearchIndex;
import com.m00ware.ftpindex.search.SearchResults;
import com.m00ware.ftpindex.web.text.WebString;
import com.m00ware.ftpindex.web.text.WebTag;
		...
		if(search != null){
			...
			for(Node node : results.getNodes()){
				...
				if(node instanceof Directory){
					if(ftp.isUp()){
						nodeText.addElement(new WebTag("<a class=\"nou\" href=\"ftp://"+StringEscapeUtils.escapeHtml(ftpStr+node.getPath())+"\">", "</a>", new WebText("+")));
					}else{
						nodeText.addElement(new WebTag("<a class=\"nou\" href=\"ftp://"+StringEscapeUtils.escapeHtml(ftpStr+node.getPath())+"\">", "</a>", new WebText("X")));
					}
					if(node.isOutDated())
						...
						nodeText.addElement(new WebString(" "));
					nodeText.addElement(new WebTag("<a href=\"/browse/"+StringEscapeUtils.escapeHtml(ftpStr+node.getPath())+"\">", "</a>", new WebText(StringEscapeUtils.escapeHtml(capStringLength(node.getName(), 50)))));
			...
			if(maxPages > 0){
				...
				while(before > 0 && current < maxPages){
					pagesLine.addElement(new WebTag("<a href=\"/search/"+StringEscapeUtils.escapeHtml(search)+"?page="+current+"\">", "</a>", new WebText(Integer.toString(current))));
					pagesLine.addElement(new WebString(" "));
					current++;
					before--;
				}
				...
				if(maxPages != 1){
					if(page != maxPages){
						pagesLine.addElement(new WebTag("<a href=\"/search/"+StringEscapeUtils.escapeHtml(search)+"?page="+maxPages+"\">", "</a>", new WebText(Integer.toString(maxPages))));					
					}
				}
