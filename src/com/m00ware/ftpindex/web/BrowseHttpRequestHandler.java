...
public class BrowseHttpRequestHandler extends BaseHttpRequestHandler
{
	...
	/* (non-Javadoc)
	 * @see com.m00ware.ftpindex.web.BaseHttpRequestHandler#handle(org.apache.http.HttpRequest, org.apache.http.HttpResponse, org.apache.http.protocol.HttpContext)
	 */
	@Override
	...
	private String ftpAction(String uri, List<WebText> lines)
	{
		...
		if(!directory.isRoot())
			headText.addElement(new WebTag("<a href=\"/browse/"+StringEscapeUtils.escapeHtml(ftpStr+directory.getParent().getPath())+"\">", "</a>", new WebText("[Up]")));
		...
		for(Node child : directory.getChildren()){
			...
			if(child instanceof Directory){
				if(ftp.isUp()){
					nodeText.addElement(new WebTag("<a class=\"nou\" href=\"ftp://"+StringEscapeUtils.escapeHtml(ftpStr+child.getPath())+"\">", "</a>", new WebText("+")));
				}else{
					nodeText.addElement(new WebTag("<a class=\"nou\" href=\"ftp://"+StringEscapeUtils.escapeHtml(ftpStr+child.getPath())+"\">", "</a>", new WebText("X")));
				}
				if(child.isOutDated())
					...
					nodeText.addElement(new WebString(" "));
				nodeText.addElement(new WebTag("<a href=\"/browse/"+StringEscapeUtils.escapeHtml(ftpStr+child.getPath())+"\">", "</a>", new WebText(StringEscapeUtils.escapeHtml(capStringLength(child.getName(), 50)))));
