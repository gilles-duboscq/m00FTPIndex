...
public class Test {
    ...
    public static void main1(String[] args) throws IOException {
        ...
        ScannerThread st = new ScannerThread(new Inet4AddressRange((Inet4Address) addr, 0xfffff800));
        st.setTimeout(85);
        st.setRescanDelay(-1);
    ...
    public static void main6(String[] args) throws IOException {
        ...
        FTP ftp = db.getFTP(InetAddress.getByName("chen-huard.rez-gif.supelec.fr"), 21);
        IndexerRunnable it = new IndexerRunnable(ftp);
        it.init();
        it.run();
        db.shutdown();
        System.out.println("Explored FTP in " + (System.currentTimeMillis() - t) + "ms, total size=" + ftp.getRoot().getSize() + "Bytes");
    }
    ...
    public static void main8(String[] args) throws IOException {
        ...
        System.out.println("Begin indexing...");
        FTP ftp = db.getFTP(InetAddress.getByName("duboscq.rez-gif.supelec.fr"), 21);
        IndexerRunnable it = new IndexerRunnable(ftp);
        it.init();
        it.run();
        db.shutdown();
    ...
    public static void main9(String[] args) throws IOException {
        ...
        System.out.println("BasicSearchIndex found " + search.getTotalResults() + " results in " + t + "ms:");
        for (Node node : search.getNodes()) {
            System.out.println(" - " + node + " " + node.getFtp() + " -> " + node.getPath());
        }
        t = System.currentTimeMillis();
        search = bsi.search("Gossip", 1, 30);
        t = System.currentTimeMillis() - t;
        System.out.println("BasicSearchIndex (cached?) found " + search.getTotalResults() + " results in " + t + "ms:");
        for (Node node : search.getNodes()) {
    ...
    public static void main10(String[] args) throws IOException {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            t = System.nanoTime() - t;
            System.out.println("Did " + 16 * 1024 + " raf.readInt() in " + t + "ns -> " + (t / (16 * 1024)) + "ns/readInt()");
            raf.close();
            raf = new RandomAccessFile("testRAF", "rw");
            count = 0;
            ...
            blackhole = count;
            t = System.nanoTime() - t;
            System.out.println("Did " + 16 * 1024 + " ByteBuffered read in " + t + "ns -> " + (t / (16 * 1024)) + "ns/read");
            raf.close();
        }
    ...
    public static void main11(String[] args) throws IOException {
        ...
        t = System.currentTimeMillis() - t;
        System.out.println("SearchIndex found " + search.getTotalResults() + " results in " + t + "ms:");
        for (Node node : search.getNodes()) {
            System.out.println(" - " + node + " " + node.getFtp() + " -> " + node.getPath());
        }
        db.shutdown();
